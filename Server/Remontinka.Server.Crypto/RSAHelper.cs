...
namespace Remontinka.Server.Crypto
{
    class RSAHelper
    {
        ...
        private static SecureString password = null;

        public static RSACryptoServiceProvider CreateProviderFromKey(string data, SecureString securePassword)
        {
            RSACryptoServiceProvider result = null;
            try
            {
                password = securePassword;
                result = DecodePEMKey( data );
            }
            catch (Exception ex) {
                throw new RSAException( "Error parse RSA key." + Environment.NewLine + ex.Message );
            }
            return result;
        }
        ...
        private static RSACryptoServiceProvider DecodePEMKey(String pemstr)
        {
            ...
            if (!string.IsNullOrWhiteSpace(pemstr))
            {
                pemstr = pemstr.Trim();
            } //if
            ...
            if(pemstr.StartsWith(pempubheader) && pemstr.EndsWith(pempubfooter))
            {
                ...
                {
                    rsaProvider = DecodeX509PublicKey(pempublickey);
                }
            }
            else if(pemstr.StartsWith(pemprivheader) && pemstr.EndsWith(pemprivfooter))
            {
                ...
                {
                    rsaProvider = DecodeRSAPrivateKey(pemprivatekey);
                }
            }
            return rsaProvider;
        }
        ...
        private static byte[] DecryptKey(byte[] cipherData, byte[] desKey, byte[] IV)
        {
            ...
            {
                throw new RSAException( "Password is not valid." );
            }
