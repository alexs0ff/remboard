...
namespace Remontinka.Server.WebPortal.Controllers
{
    public class AccountController : BaseController
    {
        ...
        {
            //отсюда форма логина http://bootsnipp.com/snippets/BaypD
            ViewBag.ReturnUrl = returnUrl;
            if (User != null && User.Identity != null && User.Identity.IsAuthenticated)
            {
                return RedirectToAction("Index", "RepairOrder");
            }
            return View();
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            try
            {
                ...
                if (RemontinkaServer.Instance.DataStore.UserDomainLoginIsExistsAndNonActivated(model.Email))
                {
                    ModelState.AddModelError("", "Данный логин существует, но не активирован. Проверьте свою почту на письмо активации.");
                    return View(model);
                }
                var result =
                    await
                ...
                switch (result)
                {
                    case SignInStatus.Success:
                        ...
                        {
                            returnUrl = Url.Action("Index", "RepairOrder");
                        }
                    ...
                    default:
                        ModelState.AddModelError("", "Ошибка входа пользователя.");
                        return View(model);
                }
            }
            catch (Exception ex)
            {
                ModelState.AddModelError("", "Ошибка входа пользователя." + ex.Message);
            }
        ...
        {
            _logger.InfoFormat("Попытка регистрации {0}",model.Login);
            if (!ModelState.IsValid)
            {
                _logger.Info("2");
                foreach (KeyValuePair<string, ModelState> keyValuePair in ModelState)
                {
                    if (keyValuePair.Value.Errors.Count > 0)
                    {
                        ...
                        {
                            _logger.ErrorFormat("Ошибка регистрации {0}:{1}",keyValuePair.Key, valueError.ErrorMessage);
                        }
        ...
        {
            return View(new ChangePasswordModel {Token = GetToken()});
        }
        ...
        public ActionResult ChangePassword(ChangePasswordModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    changePasswordSucceeded = RemontinkaServer.Instance.SecurityService.ChangePassword(token.LoginName,
